...
- **Etapa 4 -- Editor de animación (nuevo)**: crear una o más animaciones nombradas sobre la jerarquía de huesos ya definida. FreeMinecraftModels reconoce **exactamente 5 nombres** para disparo automático por estado del mob (confirmado contra su documentación): `spawn` (una vez, al crear la entidad, luego pasa a `idle`), `idle` (loop mientras la velocidad ≤ 0.08), `walk` (loop mientras la velocidad > 0.08), `attack` (una vez, regresa a `idle`), `death` (una vez, al morir). Restricción documentada importante: **si `idle` no existe, el mob no puede salir de `walk` automáticamente** -- por eso `idle` es, en la práctica, obligatorio en cuanto se define cualquier otra.
  ...
  - **Asistencia de IA, en dos niveles de confianza -- AMBOS confirmados para v1 por el spike 081**: (a) *ajuste de parámetros de preset* -- el usuario describe la animación (ej. "caminar arrastrando los pies") y la IA elige/tunea los parámetros del preset correspondiente (velocidad, amplitud, fase) en vez de inventar keyframes libres. (b) *keyframes libres* -- la IA propone una secuencia de keyframes completamente custom, fuera de los 5 presets. El spike 081 (ver `done/081-spike-calidad-ia-animacion.md`) probó ambos con Claude (puente manual) y Gemini (API gratuita) sobre 5 descripciones distintas: la calidad de contenido sostiene construir ambas HU en esta versión -- el hallazgo real no fue de calidad sino de confiabilidad del free tier (ver "Riesgos").
...
### Resueltos por el spike 081 (ejecutado y cerrado, con datos reales de Claude y Gemini)
- ~~Calidad de la IA generando keyframes coherentes en el tiempo~~ -- probada con 5 descripciones distintas contra ambos proveedores: contenido estructuralmente coherente y fiel al estilo pedido en ambos casos. **Se construyen HU-11 y HU-12 en v1**, ya no es una decisión condicional. Ver `done/081-spike-calidad-ia-animacion.md`, "Hecho".
- Hallazgo que sí cambia el diseño: el **free tier de Gemini tuvo ~20% de fallos de infraestructura** (503/timeout/429) incluso con reintentos, frente a 0 fallos del puente manual con Claude -- el modo automático (tickets 087/091) necesita reintentos con backoff antes de reportar error, y el puente manual debe presentarse como opción igual de válida por defecto, no como respaldo de segunda clase.
- Hallazgo de calibración: el umbral de "salto máximo entre keyframes" debe escalar con la duración/velocidad de la animación pedida, no ser una constante fija -- ya reflejado en el ticket 091.
