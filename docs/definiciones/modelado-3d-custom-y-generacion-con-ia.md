...
### Resueltos por el spike 080 ya ejecutado (parcial -- falta verificación visual)
- ~~Spike de validación contra FreeMinecraftModels real~~ -- ejecutado: servidor Paper 1.21.4 + FreeMinecraftModels 2.11.2 montado, `.bbmodel` corregido y parseado sin errores, pack generado correctamente (geometría + jerarquía + textura + animaciones). Falta solo la verificación visual de Marco conectando su cliente -- ver `in-process/080-spike-validar-freeminecraftmodels-real.md`, "Progreso".
