...
Hallazgo de QA (PR #71): el gate `🔍 QA Review (auto)` marcó "&lt;img&gt; sin atributo alt". Confirmado falso positivo por inspección directa del diff (`gh pr diff 71 --patch | grep -n "<img"`) -- la única aparición de `<img>` en todo el diff cae dentro de la prosa de `docs/ARQUITECTURA.md` (citando el trabajo de thumbnails del ticket 041), no en JSX real; ni `MobSelector.tsx` ni `App.tsx` tienen ningún `<img>`. Mismo patrón que el falso positivo ya documentado y reportado como bug del gate en el ticket 042 (PR #70) -- no se abrió un segundo reporte. Sin hallazgos de QA reales pendientes.
