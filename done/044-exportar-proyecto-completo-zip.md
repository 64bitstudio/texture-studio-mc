...
Hallazgo de QA (PR #72): el gate `🔍 QA Review (auto)` marcó "&lt;img&gt; sin atributo alt" -- tercera repetición del mismo falso positivo ya documentado en los tickets 042/043 (bug reportado una vez, sigue sin corregirse). Confirmado falso positivo por inspección directa del diff (`gh pr diff 72 --patch | grep -n "<img"`): todas las apariciones de `<img>` en el diff son prosa de `docs/`, y el `<img>` real de `Proyecto.tsx` no forma parte de ningún hunk modificado en este PR. Sin hallazgos de QA reales pendientes.
